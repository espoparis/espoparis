...
export default async function AdvisoryBoardPage(props: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="page-shell pb-24 pt-12 sm:pt-16 lg:pb-32 lg:pt-20">
    <header className="mx-auto max-w-4xl text-center"><p className="section-eyebrow">{board.eyebrow}</p><h1 className="mt-5 font-display text-[clamp(2.5rem,7vw,6rem)] font-medium leading-[.95] tracking-[-.05em]">{board.title}</h1><p className="mx-auto mt-6 max-w-2xl text-base leading-8 text-muted-foreground sm:text-lg">{board.description}</p></header>
    <section className="mx-auto mt-14 grid max-w-6xl gap-x-12 gap-y-8 md:grid-cols-3 lg:mt-20">{board.members.map((member) => <article key={member.name} className="border-t border-border py-7"><GraduationCap className="size-7 text-primary"/><h2 className="mt-5 font-display text-2xl font-semibold">{member.name}</h2><p className="mt-2 text-sm font-semibold text-primary">{member.country}</p><ul className="mt-5 space-y-3 text-sm leading-7 text-muted-foreground">{member.credentials.map((item) => <li key={item}>{item}</li>)}</ul></article>)}</section>
  </div>;
}
