import { Archive, CalendarRange, LockKeyhole, RefreshCw, ShieldCheck } from "lucide-react";
import type { AuthSession } from "@/server/auth/types";
import { canUseAcademicWorkspace } from "@/server/auth/policy";

export type CohortManagementCopy = {
  eyebrow: string;
  title: string;
  description: string;
  lockedTitle: string;
...
const icons = [CalendarRange, RefreshCw, Archive, ShieldCheck];

export function CohortManagementCenter({ copy, session }: { copy: CohortManagementCopy; session: AuthSession }) {
    ...
    <main className="full-bleed bg-[#f8f5ed] text-[#102c24] dark:bg-[#071a15] dark:text-[#f6f1e4]">
      <section className="page-shell py-16 pt-36 sm:pt-40 lg:py-24 lg:pt-44">
        ...
        <section className="mt-12">
          <h2 className="font-display text-3xl font-medium">{copy.principlesTitle}</h2>
          <div className="mt-6 grid gap-5 md:grid-cols-2 xl:grid-cols-4">{copy.principles.map((item,index)=>{const Icon=icons[index]??ShieldCheck;return <article key={item.title} className="rounded-[1.75rem] border border-[#dccca9] bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.04]"><Icon className="size-5 text-[#98772f]"/><h3 className="mt-4 text-lg font-semibold">{item.title}</h3><p className="mt-2 text-sm leading-7 text-[#62736d] dark:text-white/60">{item.description}</p></article>})}</div>
        </section>
        <div className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          ...
          <article className="rounded-[1.75rem] border border-[#dccca9] bg-white p-6 dark:border-white/10 dark:bg-white/[0.04]"><ShieldCheck className="size-5 text-[#98772f]"/><h2 className="mt-4 font-display text-xl font-medium">{copy.safeguardTitle}</h2><p className="mt-2 text-sm leading-7 text-[#5d6e68] dark:text-white/62">{copy.safeguardDescription}</p></article>
        </div>
      </section>
    </main>
  );
