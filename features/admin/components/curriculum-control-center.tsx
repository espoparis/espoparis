import { Archive, BookOpenCheck, CopyPlus, LockKeyhole, PencilLine, ShieldCheck } from "lucide-react";
import type { AuthSession } from "@/server/auth/types";
import { canUseAcademicWorkspace } from "@/server/auth/policy";

export type CurriculumControlCopy = {
  eyebrow: string;
  title: string;
  description: string;
  lockedTitle: string;
...
const icons = [PencilLine, CopyPlus, Archive, ShieldCheck];

export function CurriculumControlCenter({ copy, session }: { copy: CurriculumControlCopy; session: AuthSession }) {
  const decision = canUseAcademicWorkspace(session, "academic-officer");
  return (
    <main className="full-bleed bg-[#f8f5ed] text-[#102c24] dark:bg-[#071a15] dark:text-[#f6f1e4]">
      <section className="page-shell py-16 pt-36 sm:pt-40 lg:py-24 lg:pt-44">
        ...
        <section className="mt-12">
          <div className="flex items-center gap-3"><BookOpenCheck className="size-5 text-[#98772f]" /><h2 className="font-display text-3xl font-medium">{copy.principlesTitle}</h2></div>
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
          <article className="rounded-[1.75rem] border border-[#dccca9] bg-[#efe7d4] p-6 dark:border-white/10 dark:bg-white/[0.05]">
            <h2 className="font-display text-xl font-medium">{copy.lifecycleTitle}</h2>
            <ol className="mt-4 space-y-3">{copy.lifecycle.map((step, index) => <li key={step} className="flex gap-3 text-sm leading-6"><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-[#0f4738] text-xs font-semibold text-[#e9cd8a]">{index + 1}</span><span>{step}</span></li>)}</ol>
          </article>
          <article className="rounded-[1.75rem] border border-[#dccca9] bg-white p-6 dark:border-white/10 dark:bg-white/[0.04]">
            ...
            <h2 className="mt-4 font-display text-xl font-medium">{copy.controlsTitle}</h2>
            <p className="mt-2 text-sm leading-7 text-[#5d6e68] dark:text-white/62">{copy.controlsDescription}</p>
          </article>
        </div>
      </section>
    </main>
