    ...
    <div className="page-shell pb-24 pt-12 sm:pt-16 lg:pb-32 lg:pt-20">
      ...
      <section className="mx-auto mt-14 grid max-w-6xl gap-6 lg:grid-cols-[.9fr_1.1fr]">
        <div className="rounded-sm bg-[#082e24] p-7 text-white sm:p-9">
          <div className="flex size-12 items-center justify-center rounded-sm bg-white/10"><GraduationCap className="size-6 text-[#e2c27b]" /></div>
          <h2 className="mt-6 font-display text-3xl font-semibold">{copy.hierarchyTitle}</h2>
          <div className="mt-7 space-y-3">
            {copy.hierarchy.map((item, i) => (
              <div key={item} className="flex items-center gap-4 rounded-sm border border-white/10 bg-white/[.05] px-4 py-3">
                <span className="flex size-8 items-center justify-center rounded-sm bg-[#d7b56d] text-xs font-bold text-[#082e24]">{i + 1}</span>
        ...
        <div className="rounded-sm border border-border bg-card p-7 sm:p-9">
          ...
          <div className="mt-7 grid gap-4 sm:grid-cols-2">
            {copy.features.map((f) => {
              ...
              return <article key={f.title} className="border-t border-border p-5"><Icon className="size-5 text-primary" /><h3 className="mt-4 font-semibold">{f.title}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{f.description}</p></article>;
            })}
          </div>
          <p className="mt-6 rounded-sm bg-secondary/55 p-4 text-sm leading-6 text-muted-foreground">{copy.accessNote}</p>
        </div>
      </section>
      ...
      <section className="mx-auto mt-16 max-w-6xl">
        ...
        <div className="mt-8 flex gap-2 overflow-x-auto pb-2">
          {years.map((year, index) => (
            <button key={year.year} aria-pressed={selectedYear === index} onClick={() => setSelectedYear(index)} className={`shrink-0 rounded-sm border px-4 py-2 text-sm font-medium transition ${selectedYear === index ? "border-primary bg-primary text-primary-foreground" : "border-border bg-background hover:border-primary/50"}`}>
              {year.year}
            </button>
          ))}
        </div>
        ...
        {activeYear && (
          <div className="mt-5 rounded-sm border border-border bg-card p-6 sm:p-8">
            ...
            <div className="mt-6 grid gap-5 lg:grid-cols-2">
              {activeYear.semesters.map((semester) => (
                <article key={semester.label} className="border-t border-border p-5">
                  <h4 className="font-semibold">{semester.label}</h4>
                  <div className="mt-4 space-y-2">
                    {semester.courses.map((course) => (
                      <div key={course} className="flex items-center justify-between gap-3 rounded-sm bg-secondary/45 px-3 py-3 text-sm">
            ...
            {activeLessons.length ? (
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {activeLessons.map((lesson) => (
                  <article key={lesson.id} className="rounded-sm border border-border/70 bg-secondary/20 p-4">
              ...
              <div className="mt-6 flex flex-col gap-3 rounded-sm border border-dashed border-border bg-secondary/20 p-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-muted-foreground">{copy.recordingsPending}</p><Link href={`/${locale}/student`} className="shrink-0 rounded-sm border border-primary/30 bg-background px-4 py-2 text-sm font-semibold text-primary transition hover:bg-primary hover:text-primary-foreground">{copy.portalLabel}</Link></div>
            )}
          </div>
        )}
      </section>
