...
export function AcademicProgramGuide({ copy }: { copy: AcademicProgramCopy }) {
  ...
  return <div className="page-shell pb-24 pt-12 sm:pt-16 lg:pb-32 lg:pt-20">
    <header className="mx-auto max-w-4xl text-center">
      ...
      <div className="mx-auto mt-8 flex max-w-3xl flex-wrap justify-center gap-2">{copy.facts.map(f=><span key={f} className="rounded-sm border border-primary/15 bg-primary/[.05] px-4 py-2 text-sm font-medium">{f}</span>)}</div>
    </header>

    <motion.section {...enter} className="mx-auto mt-16 max-w-6xl rounded-sm bg-[#082e24] p-7 text-white sm:p-10">
    ...
    <section className="mx-auto mt-20 max-w-6xl">
      ...
      <div className="mt-10 space-y-6">{copy.years.map((y,i)=><motion.article {...enter} key={y.year} className="overflow-hidden rounded-sm border border-border bg-card">
